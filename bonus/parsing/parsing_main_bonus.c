...
/*   Updated: 2024/02/26 12:36:21 by seongjko         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	parsing->is_heredoc = !(ft_strncmp(argv[1], "here_doc", 9));
	if (parsing->is_heredoc)
	{
		...
		parsing->cmd_cnt = argc - 3;
	parsing->splitted_envp_path = ft_split(ft_getenv("PATH", envp), ':');
	parsing->cmd_argv = make_command_argv(argv, 2 + parsing->is_heredoc, \
	parsing->cmd_cnt, argc);
